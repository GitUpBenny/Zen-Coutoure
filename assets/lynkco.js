...
//               }
              ...
              lynkco.doAjaxAddToCart(data, handle);
            }
          }
          return false;
        });
...
//               }
    ...
    doAjaxAddToCart: function(data, handle) {
      $.ajax({
        type: "post",
        url: "/cart/add.js",
        data: data,
          ...
          $.get(window.router + '/cart?view=json', function(data) {
            ...
            if( window.dpl_upsell ){
              ...
              if( handle != _handle1){
                ...
                $('.ajax-success-modal .ajax-load-content').load(_handle + '/collections/all?view=related');
              }
            }
    ...
    initQuickView: function() {
      $('body').off('click.initQuickView', '.quickview-button a').on('click.initQuickView', '.quickview-button a', function(e) {
        ...
        jQuery.getJSON(window.router + '/products/'+product_handle+'.js', function(product) {
          ...
          $('.quick-view').find('form').attr('id', 'qv-add-to-cart' + product.id); 
          $('.quick-view').find('button.add-to-cart-btn').attr('data-form-id','#qv-add-to-cart' + product.id); 
      ...
      if(width < 1025) {
        ...
        $('.menu-mobile .menuToggle').off('click').on('click', function(e) {
          ...
          if($('html').hasClass('menu-open')) {
            ...
            $('.navigation .site-nav .mobile-toggle-open').removeClass('mobile-toggle-open');
          }
...
//         };
      ...
      var menuMbTitle = $('li.menu-mb-title');
      var menuMbTitle1 = $('.dropdow-lv2, .dropdown:not(.mega-menu) .site-nav-dropdown');
      menuMbTitle1.on('click', function(e) {
      	e.stopPropagation();
      });

      menuMbTitle.off('click.closeMenu').on('click.closeMenu', function(e) {
        e.preventDefault();
//         $(this).parent().parent().removeClass('mobile-toggle-open');
        $(this).parents('.inner').removeClass('mobile-toggle-open');
        if (!$(this).parents('.item').hasClass('.mega-menu')) {
        	$(this).parent().parent().removeClass('mobile-toggle-open');
        }
        if ($('.mobile-toggle-open').length > 0) {
          $(this).closest('.navigation').addClass('overflow');
        } else {
          $(this).closest('.navigation').removeClass('overflow');
        }
    ...
    initDropdownMenuMobile: function() {
      if(window.innerWidth < 1025) {
        ...
        var iconDropdown = $('.site-nav li.dropdown:not(.menu-mb-title), .site-nav .inner');
...
//             $(this).find('.site-nav-dropdown').removeClass('open-menu');
...
//             $(this).find('.site-nav-dropdown').addClass('open-menu');
            $(this).addClass("mobile-toggle-open");

            if($('.bottom-header').hasClass('is_stuck')){
              $(this).find('.site-nav-dropdown').css('top',heightbottom);
              $(this).find('.dropdow-lv2').css('top',heightbottom);
            }
            else{
              $(this).find('.site-nav-dropdown').css({ "top": heightHeader});
              $(this).find('.dropdow-lv2').css({ "top": heightHeader});
            };
    ...
    dropDownSubCategory: function() {
      $('.sidebar .caret').on('click', function(e){
        ...
        if($(mn).hasClass('active')) {
          ...
          $(mn).removeClass('active open');
        }
        else{
          $(this).next().slideDown(200);
          $(mn).addClass('active open');
        }
      });
    },
    ...
    initStickyAddtoCart: function(){
      ...
      var t = $('.sticky_form .pr-swatch[data-value="'+p+'"]').html();
      $('.pr-selectors .pr-active').html(t);
    ...
    initGroupedAddtoCart: function(){      
      ...
      $('#grouped-add-to-cart').on('click', function(e) {
        ...
        Shopify.queue = [];
        jQuery('.group_product').each(function() {
          var DataId = $(this).find('input[type=hidden]').attr('value'),
              DataQty = parseInt($(this).find('input[name=group_quantity]').val());
          if ( DataQty > 0 && DataId !== '') {
            Shopify.queue.push({
              variantId: DataId,
    ...
    initWishList: function(e){      
      ...
      var dataWishlist = function(ProductHandle) {
        jQuery.getJSON(window.router + '/products/'+ProductHandle+'.js', function(product) {
          ...
          productHTML += '<div class="col-md-3 wishlist-action"><form action="/cart/add" method="post" class="variants" enctype="multipart/form-data" id="wishlist-product-form-' + product.id +'">';
          productHTML += '<input type="hidden" name="id" value="'+ product.variants[0].id +'" />  ';
          if (product.available) {
            if (product.variants.length == 1) {
              productHTML += '<input class="btn add-to-cart-btn" type="submit" value="'+window.inventory_text.add_to_cart+'" data-form-id="#wishlist-product-form-' + product.id +'"/>'; 
            } 
            if (product.variants.length > 1){
